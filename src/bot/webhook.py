...
async def _set_user_state(redis: aioredis.Redis, max_user_id: int, state: str) -> None:
    await redis.set(f"state:{max_user_id}", state, ex=USER_STATE_TTL)


async def _get_user_state(redis: aioredis.Redis, max_user_id: int) -> str | None:
    return await redis.get(f"state:{max_user_id}")


async def _clear_user_state(redis: aioredis.Redis, max_user_id: int) -> None:
    await redis.delete(f"state:{max_user_id}")
...
async def _handle_message(
    ...
    state = await _get_user_state(redis, max_user_id)
    ...
    if text.startswith("/"):
        return await _handle_command(text, chat_id, max_user_id, user, db_pool, redis)
...
async def _handle_command(
    ...
    if cmd == "/ticket":
        await _set_user_state(redis, max_user_id, "awaiting_ticket")
        await max_client.send_message(
            chat_id,
...
async def _create_ticket_from_message(
    ...
    *,
    ...
    await _clear_user_state(redis, user["max_user_id"])
    ...
    if len(text) < 5:
        await max_client.send_message(
            chat_id,
            "Описание слишком короткое (минимум 5 символов). Попробуйте ещё раз: /ticket",
        )
...
def _extract_photo(message: dict) -> str | None:
    ...
    for att in attachments:
        ...
        if att.get("type") in ("image", "photo"):
            token = _safe_get(att, "payload", "token") or _safe_get(att, "payload", "url")
            if token:
                return str(token)
    return None
